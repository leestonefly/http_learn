    ...
    for (var j = 0; j < balls.length; j++) {
        if (!(this === balls[j])) {
            ...
            if (distance === this.size + balls[j].size) {
                ...
                this.velX = ((balls[j].size ** 2 - this.size ** 2) * this.velX + 2 * balls[j].size ** 2 * balls[j].velX) / (balls[j].size ** 2 + this.size ** 2);
                balls[j].velY = ((balls[j].size ** 2 - this.size ** 2) * balls[j].velY + 2 * this.size ** 2 * this.velY) / (balls[j].size ** 2 + this.size ** 2);
                this.velY = ((balls[j].size ** 2 - this.size ** 2) * this.velY + 2 * balls[j].size ** 2 * balls[j].velY) / (balls[j].size ** 2 + this.size ** 2);

            } else if (distance < this.size + balls[j].size) {
                let x = this.velX;
                let y = this.velY;
